...
export const HypertensionManagement = () => {
  ...
  const generateRecommendations = (): TreatmentRecommendation => {
    ...
    const lifestyleRecs = [
      ...
      'Alcohol limitation (â‰¤2 drinks/day men, â‰¤1 drink/day women)'
    ];
    ...
    if (sbp >= 140 || dbp >= 90 || (sbp >= 130 && (hasCKD || hasDiabetes || hasASCVD))) {
      ...
      if (hasCKD || hasDiabetes) {
        ...
        if (!hasAllergyACE) {
          medicationRecs.push('â€¢ ACE inhibitor (e.g., lisinopril 10-40mg daily)');
        }
        medicationRecs.push('â€¢ ARB (e.g., losartan 50-100mg daily)');
        medicationRecs.push('â€¢ Calcium channel blocker (e.g., amlodipine 5-10mg daily)');
        medicationRecs.push('â€¢ Thiazide diuretic (e.g., HCTZ 25-50mg daily)');
